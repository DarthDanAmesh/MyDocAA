...
export default function ChatInterface() {
  ...
  useEffect(() => {
    scrollToBottom();
    // Fetch chat history: /api/chat/history (implement in backend)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: 'assistant', content: data.content, ragContext: data.ragContext }]);
    } catch (error) {
      console.error('Error:', error);
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: 'Sorry, I encountered an error.', model: selectedModel },
      ]);
    } finally {
      setIsLoading(false);
    }
